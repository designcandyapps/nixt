export default defineEventHandler(async(event)=>{
  ...
  const query="green circles";
  //const query=getQuery(event);
  const imageUrl=query.url as string;

  if(!imageUrl){return new Response("Invalid URL",{status:400})}
  //alert("Q: "+query);
  const ep=`https://api.gettyimages.com/v3/search/images?phrase=${encodeURIComponent(query)}&page=${page}&page_size=${pageSize}`;
  try{const response=await fetch(ep,{
    headers:{
